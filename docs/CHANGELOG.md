...
- Stream D: Added `devel/protocol_walkthrough.mjs`, a two-pass Playwright walkthrough test. Pass A walks the explicit `nextId` chain from the first step to null, calling `completeStep(id)` in sequence and screenshotting each step into `build/walkthrough/NN_<id>.png`. Pass B validates trigger coverage: reads `window.__protocolValidation` (set by `validateTriggerCoverage` on page load) and diffs `PROTOCOL_STEPS` against `window.__registeredTriggers`. The test asserts final state: `completedSteps.length === 25`, `stepsOutOfOrder === 0`, `activeStepId === null`. Invoked via `bash walkthrough.sh`, which builds the game first.
- Stream D: Extended `devel/test_protocol_flow.mjs` with two `nextId` schema checks: "Every step has a nextId field" (all 25 steps must define nextId as string | null | function), "Following nextId from the first step visits all PROTOCOL_STEPS" (walk the chain and verify it covers all 25 steps). Test count: 13/13 passing (was 11/11).
...
- Stream D: Created `devel/protocol_walkthrough.mjs`, a two-pass Playwright test. Pass A walks the explicit `nextId` chain, calling `completeStep(id)` for each of the 25 steps in sequence and screenshotting into `build/walkthrough/NN_<id>.png` where NN is a zero-padded 2-digit index. Pass B validates trigger coverage by reading `window.__protocolValidation` (set by `validateTriggerCoverage()` on page load) and diffing `PROTOCOL_STEPS` ids against `window.__registeredTriggers`. Invoked via `bash walkthrough.sh`, which builds the game first. Final-state assertions: `completedSteps.length === 25`, `stepsOutOfOrder === 0`, `activeStepId === null`. **BLOCKED**: Prior streams did not complete the migration of `parts/feed_cells.ts` lines 80 and 175 from `completeStep()` to `triggerStep()`. Without this fix, `validateTriggerCoverage()` reports missing coverage for `aspirate_old_media` and `neutralize_trypsin`, causing the walkthrough to fail. Orchestrator action required: edit feed_cells.ts or authorize Stream D to do so.
